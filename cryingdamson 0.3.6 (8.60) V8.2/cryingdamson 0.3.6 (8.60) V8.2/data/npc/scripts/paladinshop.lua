...
function creatureSayCallback(cid, type, msg)
	...
	local mlprice = (getPlayerStorageValue(cid, 8081) * 400)
	local shieldingprice = (getPlayerStorageValue(cid, 8082) * 85)
	local distanceprice = (getPlayerStorageValue(cid, 8086) * 35)
	...
	if (isPaladin(cid)) then
		...
		if(msgcontains(msg, 'distance') or msgcontains(msg, 'ranged')) then
			selfSay('You want to buy a distance upgrade for {' .. distanceprice .. '} gold?', cid)
		...
		if(msgcontains(msg, 'yes')) and (talkState[talkUser] == 4) then
			if doPlayerRemoveMoney(cid, mlprice) then
					...
					setPlayerStorageValue(cid, 8086, getPlayerStorageValue(cid, 8086) + 1)
					talkState[talkUser] = 0
			else
		...
		npcHandler:say("I only help the mages here, go see {Borris} the knight or {Bealda} the mage.", cid)
